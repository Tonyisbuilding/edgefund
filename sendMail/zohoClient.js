...
const getOAuthDomain = () => (process.env.ZOHO_OAUTH_DOMAIN || "https://accounts.zoho.com").replace(/\/$/, "");

// 🔑 Force Zoho Mail domain
...
const sendZohoMail = async ({ subject, html, text, to, cc, bcc, replyTo, from }) => {
  ...
  if (!toList) throw new Error("Zoho mail send failed: no recipients provided.");
  ...
  if (cc) payload.ccAddress = buildRecipientField(cc);
  if (bcc) payload.bccAddress = buildRecipientField(bcc);
  if (replyTo) payload.replyToAddress = replyTo;

  if (!payload.content) throw new Error("Zoho mail send failed: no content supplied.");
  ...
  try {
    ...
    if (process.env.ZOHO_DEBUG === "true") {
      console.log("Zoho mail sent", { endpoint, payload, response: response.data });
    }

    return response.data;
  } catch (error) {
    const message = error.response?.data || error.message;
    console.error("Zoho Mail API send failed", { endpoint, message, payload });
    throw error;
  }
};
